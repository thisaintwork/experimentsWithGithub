...
<!--
 ...
 Postcond:
  Summary: This produces a list of labels from the XML. This is magic
    ...
    <!--
    Input: all of the possible field names for this project
   Output: an xml snippet with a unique list of field names.
  Precond:
 Postcond:
  Summary: This produces a list of labels from the XML. This is magic
           stolen from stack overflow
    -->
    <xsl:variable name="uniqProjFieldNamesList">
        <uniqProjFieldNamesList>
            <xsl:for-each select="//organization/projectV2/items/nodes/fieldValues/nodes/field/name[not(.=preceding::*)]">
                <uniqProjFieldName>
                    <!--<xsl:text>'</xsl:text>-->
                    <xsl:value-of select="."/>
                    <!--<xsl:text>'</xsl:text>-->
 ...
 Postcond:
  Summary:
    ...
    <xsl:template match="/" >
        ...
        <!--
        Append the list of field names to the header row  
        -->       
        <xsl:for-each select="$uniqProjFieldNamesList/uniqProjFieldNamesList/uniqProjFieldName">
    ...
    <xsl:template match="*/projectV2/items/nodes" >
        ...
        <!-- Put values with the list of field names
      Summary: 
     ...
     Postcond:
        -->
        <xsl:variable name="thisItemFieldListElements" select="./fieldValues/nodes/field/*" />
        
        <xsl:for-each select="$uniqProjFieldNamesList/uniqProjFieldNamesList/uniqProjFieldName">
            <xsl:variable name="thisUniqListElement" select="." />
            <xsl:for-each select="$thisItemFieldListElements">
                
                <xsl:choose>   
                    <xsl:when test='string(.) = string($thisUniqListElement)'>
                        <xsl:value-of select='concat(string(../../name),string(../../text))'/>
                    </xsl:when>
                </xsl:choose>
            </xsl:for-each>
            <xsl:text>&#9;</xsl:text>
